...
WINNER_PIECE_NUMBER = 1
LOSER_PIECE_NUMBER = 0.5
EMPTY_PIECE_NUMBER = 0

DATA_FILES_NAME = [f"csv_files/wthor ({i}).csv" for i in range(46)]
...
def main():
    ...
    for filename in DATA_FILES_NAME:
        for transcript in extract_data.extract(filename):
            ...
            if current_data_count == DATA_COUNT_PER_FILE:
                np.savez(f"data/wthor-{current_file_count}", np.array(save_board_data), np.array(save_moves_data))
                current_data_count = 0
