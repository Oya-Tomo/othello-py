...
class Othello:
                ...
                for i in range(1, 8):
                    match self.current_board[y + i * y_dir][x + i * x_dir]:
                        case self.self_num:
                            break
                        case self.opnt_num:
                            self.current_board[y + i * y_dir][x + i * x_dir] = self.self_num
        else:
                ...
                for i in range(1, 8):
                    match self.current_board[y + i * y_dir][x + i * x_dir]:
                        case self.opnt_num:
                            break
                        case self.self_num:
                            self.current_board[y + i * y_dir][x + i * x_dir] = self.opnt_num

    def can_put(self, x: int, y: int) -> list: # return directions that you can flip stones.
    ...
    def judge_winner(self) -> OthelloPiece:
        ...
        if self_count == opnt_count:
            ...
            return OthelloPiece.White if self.color == OthelloPiece.Black else OthelloPiece.Black
