...
class Game:
    def __init__(self):
        ...
        self.model = tf.keras.models.load_model("models/model5")
    ...
    def render_splash(self):
        ...
        while frame_count < 30 * 5:
            ...
            self.screen.blit(splash_image, ((1000 - image_size[0]) / 2, (850 - image_size[1]) / 2))
    ...
    def render_ready(self):
        text_1 = pygame.font.SysFont("Noto Sans CJK JP", 50).render("Select your piece color.", True, WHITE)
        text_2 = pygame.font.SysFont("Noto Sans CJK JP", 40).render("Black: Press B", True, WHITE)
        text_3 = pygame.font.SysFont("Noto Sans CJK JP", 40).render("White: Press W", True, WHITE)
        text_4 = pygame.font.SysFont("Noto Sans CJK JP", 40).render("Quit: Press ESC", True, WHITE)
        ...
        while True:
            ...
            pygame.draw.rect(self.screen, GREEN, Rect(100, 300, 300, 300), border_radius=10)
            pygame.draw.rect(self.screen, GREEN, Rect(600, 300, 300, 300), border_radius=10)
    ...
    def render_color_selected(self):
        frame_count = 0
        text_1 = pygame.font.SysFont("Noto Sans CJK JP", 50).render(f"Your color is {self.player_color}. Good Luck !", True, WHITE)

        while frame_count < 30 * 2:
            self.screen.fill(BLACK)
            self.render_bg_pattern()
            render_surf(self.screen, text_1, 1 / 2, 100)
            pygame.draw.rect(self.screen, GREEN, Rect(350, 300, 300, 300), border_radius=10)
    ...
    def render_match_start(self):
        ...
        while frame_count < 30 * 4:
            ...
            if frame_count < 30:
                text = pygame.font.SysFont("Noto Sans CJK JP", 100).render("3", True, YELLOW)
            elif frame_count < 60:
                text = pygame.font.SysFont("Noto Sans CJK JP", 100).render("2", True, YELLOW)
            elif frame_count < 90:
                text = pygame.font.SysFont("Noto Sans CJK JP", 100).render("1", True, YELLOW)
            else:
                text = pygame.font.SysFont("Noto Sans CJK JP", 100).render("start !", True, YELLOW)
    ...
    def render_match(self):
        text_pass_ai = pygame.font.SysFont("Noto Sans CJK JP", 100).render("AI pass", True, PURPLE)
        text_pass_player = pygame.font.SysFont("Noto Sans CJK JP", 100).render("You pass", True, PURPLE)
        text_thinking1 = pygame.font.SysFont("Noto Sans CJK JP", 50).render("thinking.", True, YELLOW)
        text_thinking2 = pygame.font.SysFont("Noto Sans CJK JP", 50).render(" thinking..", True, YELLOW)
        text_thinking3 = pygame.font.SysFont("Noto Sans CJK JP", 50).render("  thinking...", True, YELLOW)
        frame_count = 0
        if self.player_turn:
    ...
    def render_match_end(self):
        ...
        while True:
            ...
            if frame_count <= 30:
                text_end = pygame.font.SysFont("Noto Sans CJK JP", 130 - frame_count).render("Game End", True, YELLOW)
                text_end.set_alpha(frame_count * 8.5)
            
            render_surf(self.screen, text_end, 1 / 2, 350)
            if frame_count >= 60:
                text_guide = pygame.font.SysFont("Noto Sans CJK JP", 50).render("Show Result: Press SPACE", True, YELLOW)
    ...
    def render_result(self):
        ...
        if winner == othello.OthelloPiece.Black:
            text_res = pygame.font.SysFont("Noto Sans CJK JP", 70).render("Winner : Black", True, YELLOW)
        elif winner == othello.OthelloPiece.White:
            text_res = pygame.font.SysFont("Noto Sans CJK JP", 70).render("Winner : White", True, YELLOW)
        else:
            text_res = pygame.font.SysFont("Noto Sans CJK JP", 70).render("Judge : Draw", True, PURPLE)
        text_guide = pygame.font.SysFont("Noto Sans CJK JP", 50).render("New Game: Press N", True, WHITE)
        while True:
            self.screen.fill(BLACK)
            self.render_bg_pattern()
    ...
    def render_pieces(self):
        for y in range(8):
            for x in range(8):
                if self.black_player.current_board[y][x] == 1:
                    pygame.draw.circle(self.screen, GREEN_SHADOW, (100 * x + 155, 100 * y + 80), 39)
                    pygame.draw.circle(self.screen, BLACK, (100 * x + 150, 100 * y + 75), 40)
                elif self.black_player.current_board[y][x] == 0.5:
                    pygame.draw.circle(self.screen, GREEN_SHADOW, (100 * x + 155, 100 * y + 80), 39)
                    pygame.draw.circle(self.screen, WHITE, (100 * x + 150, 100 * y + 75), 40)
                else:
                    pass
    ...
    def render_field(self):
        ...
        for i in range(1, 8):
            pygame.draw.line(self.screen, GREEN_BLACK, (100 * i + 100, 25), (100 * i + 100, 825), width=3)
            pygame.draw.line(self.screen, GREEN_BLACK, (100, 100 * i + 25), (900, 100 * i + 25), width=3)
        ...
        for y in range(17):
            for x in range(20):
                if (x + y) % 2 == 0:
                    ...
                    pygame.draw.rect(self.screen, BLUE_BLACK, Rect(x * 50, y * 50, 50, 50))
    ...
    def _get_predict(self) -> tuple[int, int]:
        if self.player_color == "black":
            res = self.model.predict(np.array([
                self.white_player.current_board
            ], np.float32))[0]
            while True:
                x = np.argmax(res) % 8
                y = int(np.argmax(res) / 8)
                dirs = self.white_player.can_put(x, y)
                if len(dirs) > 0:
                    break
                else:
            ...
            res = self.model.predict(np.array([
                self.black_player.current_board
            ], np.float32))
            while True:
                x = np.argmax(res[0]) % 8
                y = int(np.argmax(res[0]) / 8)
                dirs = self.black_player.can_put(x, y)
                if len(dirs) > 0:
                    break
                else:
